---
title: "complete R script for protein-protein interaction data analysis"
...
for (k in o[1:length(o)]){           # change k  1:4445
      ...
      write.csv(links_all, paste("OUTPUT/poxa/", input$Species[k], "_chrom_data.csv", sep=""))
   ...
   ######## get plasmid's gene interactions # select plasmid-associated interactions
    for (jj in 1:length(accessID2)){
      ...
      write.csv(mapped_p, paste("OUTPUT/poxa/", input$Species[k], "_", access_ID2, ".csv", sep=""))
     ...
     if(dim(mapped_p)[1] > 0){ # if genes found
      ...
      pdf(paste("OUTPUT/poxa/", input$Species[k], "_", access_ID2, "_plasmid-chrom_subnetwork.pdf", sep=""),
          width=16, height=20)
      ...
      pdf(paste("OUTPUT/poxa/", input$Species[k], "_", access_ID2, "_intraplasmid_subnetwork.pdf", sep=""),
          width=16, height=20)
      string_db$plot_network(string_db$mp(p1)) # make plot 
      dev.off()    # make a plot of the network for fun
      
      #  plot histogram of score distribution
      pdf(paste("OUTPUT/poxa/", input$Species[k], "_", access_ID2, "_chrom_histogram.pdf",sep=""),
          width=8, height=10)
...
# save as CSV
write.csv(table_jaccard_pek.2,"OUTPUT/pek204_4419/table_jaccard_pek.2.csv")
...
#read in csv file
jaccard_table <-read.csv("OUTPUT/pek204_4419/table_jaccard_pek.2.csv")
str(jaccard_table)
...
#find mean,sd
t.ratio <- log2(jaccard_table$Ratio) #log2 transformation of jaccard ratio
raw.metric <- na.omit(t.ratio) #remove NA 
mean <- mean(raw.metric)
sd <- sd(raw.metric)
...
for(i in 1:length(t.ratio)){ #adjustment: final = (metric - mean)/SD
  final.metric[i] <- (t.ratio[i]- mean )/sd
}

#add adjustment as a column to jaccard_table
jaccard_table <- jaccard_table %>% mutate(Adjustment = final.metric)
metric.final <- na.omit(final.metric)
mean.f <- mean(metric.final)
sd.f <- sd(metric.final)
...
plot(metric.final,norm.f,
     ...
     cex=0.3, cex.main=1,cex.axis=1, cex.lab=1, pch=19)

write.csv(jaccard_table,"OUTPUT/pek204_4419/jaccard_table_ratio.csv")

#Adjusted association with pEK204 vs The Number of Interacting Genes with pEK204
#read in data of interacting genes
genes_table <-read.csv("OUTPUT/pek204_4419/table_interacting_genes_pek.2.csv")
#combine the jaccard final metric and interacting genes of each chrom
final_data <- cbind(genes_table$Chrom, as.data.frame(final.metric), genes_table$EU935740)
...
write.csv(all, "OUTPUT/pek204_4419/final.metric-pek204.interacting.genes.csv")

#####analyze pil genes in chromosomes
#read in the list of chromosome with pil operons
pil_list <- read.csv("TDA_Summer2021/list.pil.csv")
#create a new column for chromosome names  
pil_list$chrom <- gsub("_EU935740",  replacement="", pil_list$Species)
pp <- na.omit(pil_list[,c("chrom","Count")]) #341 samples
#extract the chroms that are present in both "all" and "pp"
set1 <- subset(all, chrom %in% pp$chrom) #316 samples #25 samples are not present in "all"
...
#plot 
png("final_plot.png")
figure1 <- ggplot(data = com, aes(com$final.metric, com$interacting.genes,colour= pil_count)) +
  geom_point(alpha=0.5,size=pointSize) +
  scale_colour_gradient(low = "steelblue", high = "red")+
  xlab("Adjusted association with pEK204") +  
  ylab("The Number of Interacting Genes with pEK204")+
  theme_bw()+
  geom_text_repel(data = com,aes(x = com$final.metric, y = com$interacting.genes, label = species),
                  size=3, point.padding = 0, # additional padding around each point
